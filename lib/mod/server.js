...
function html ( /*code, */ out ) {
  var code = (arguments.length === 2) ? arguments[0] : 200;
...
exports.start = function ( ip ) {
  ...
  var port = /^\:?\d+$/.test(ip)
            ...
            : 80;
  var addr = /^(\d+\.\d+\.\d+\.\d+)(\:|$)/.test(ip)
            ? ip.replace(/\:.*$/,'') || "127.0.0.1"
            : "127.0.0.1";
  var views = [];
  var s = http.createServer(function (req, res) {
    var rt, matched = false;
    res.html = html;
    res.text = text;
    res.json = json;
    res.file = file;
    ...
    for (var m, i=0,l=views.length; i<l; i++) {
      var r = views[i];
      if ( req.method === r[0] && (m = r[1].exec( path )) ) {
        //;;;console.log( req.method, 200, req.url );
        matched = true;
        var a = [ req, res ];
        for (var mi=1,ml=m.length; mi<ml; mi++) { a.push( m[mi] ); }
        rt = r[2].apply( null, a );
        break;
      }
    }
 ...
 * 
 */
var mimetypes = exports.mimetype = {
   "3gp"     : "video/3gpp",
