...
function get_tag_uri ( iurl, date ) {
  ...
  var d = date ? dateutil.format( date, ',Y-m-d' ) : '';
  return 'tag:' + u.hostname + d + ':' + u.pathname + (u.hash||'').replace('#','/');
}

function rfc2822_date ( date ) {
  return dateutil.format( date, 'r' );
}

function rfc3339_date ( date ) {
  return dateutil.format( date, 'c' );
}
...
Feed.prototype = {
  ...
  add_root_elements: function ( parentNode ) {
    ...
    parentNode.appendChild( new XMLNode( "lastBuildDate", rfc2822_date( this.latest_post_date() ) ) );
...
Rss201rev2Feed.prototype = extend(new Feed, {
  ...
  add_item_elements: function ( parentNode, container ) {
    ...
    if ( container.pubdate != null ) {
      parentNode.appendChild( new XMLNode( "pubDate", rfc2822_date(container.pubdate) ) );
    }
...
Atom1Feed.prototype = extend(new Feed, {
  ...
  add_root_elements: function ( parentNode ) {
    ...
    p.appendChild( new XMLNode( "updated", rfc3339_date(this.latest_post_date()) ) );
    if ( f.author_name ) {
  ...
  add_item_elements: function ( parentNode, container ) {
    ...
    if ( c.pubdate ) {
      p.appendChild( new XMLNode( "updated", rfc3339_date(c.pubdate) ) );
    }
    // Author information.
    if ( c.author_name ) {
      var auth = parentNode.appendChild( new XMLNode('author') );
