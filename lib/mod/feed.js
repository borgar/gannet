...
XMLNode.prototype = {
  ...
  toString: function (indent) {
    indent = indent || 0;
    let r; let attr = ''; const a = []; const ws = Array(indent + 1).join('  ');
...
Feed.prototype = {
  ...
  add_item: function (title, link, description, author_email,
    ...
    ttl) {
    const item = (typeof title === 'object') ? title : {
      title: title,
      link: iri_to_uri(link),
      description: description,
      author_email: author_email,
  ...
  toString: function () {
    const document = new XMLNode('rss', this.rss_attributes());
    const channel = new XMLNode('channel', this.root_attributes());
  ...
  write_items: function (parentNode) {
    this.items.forEach(function (item) {
      const itemnode = new XMLNode('item', this.item_attributes(item));
...
Rss201rev2Feed.prototype = extend(new Feed(), {
  ...
  add_item_elements: function (parentNode, container) {
    ...
    if (container.author_name && container.author_email) {
      const auth = container.author_email + ' (' + container.author_name + ')';
...
Atom1Feed.prototype = extend(new Feed(), {
  ...
  toString: function () {
    const document = new XMLNode('feed', this.root_attributes());
  ...
  add_root_elements: function (parentNode) {
    const f = this.feed; const p = parentNode;
    ...
    if (f.author_name) {
      const auth = parentNode.appendChild(new XMLNode('author'));
  ...
  write_items: function (parentNode) {
    this.items.forEach(function (item) {
      const itemnode = new XMLNode('entry', this.item_attributes(item));
      this.add_item_elements(itemnode, item);
      parentNode.appendChild(itemnode);
    }, this);
  },
  ...
  add_item_elements: function (parentNode, container) {
    const p = parentNode; const c = container;
    ...
    // Unique ID.
    const unique_id = c.unique_id || get_tag_uri(c.link, c.pubdate);
