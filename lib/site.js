...
function Site ( settings, verbose, smart_writes ) {
  ...
  read: function () {
    ...
    for (var i=0; i<files.length; i++) {
      ...
      if ( ext in this.converters ) {
        ...
        console.log( 'Unknown file type:', files[i].filename );
      }
    }
    ...
    , path.join( this.resolve_path_setting( 'THEME_PATH' ), 'static' )
      ...
      read_files( stat ).forEach(function ( source ) {
        ...
        if ( this.smart_writes &&
             dst in this.existing_files &&
  ...
  render: function () {
      ...
      arr.forEach(function ( item ) {
        ...
        if ( outfn in this.existing_files ) {
          ...
          var lastdt = this.existing_files[ outfn ].mtime
            , ndate = Math.max( item.mtime || item.date
                              , _template.mtime
                              , this.settings.mtime )
            ;
    ...
    'index archives tags categories authors'.split(' ')
      ...
      .forEach(function ( key ) {
        var _template = this._templates[ key ]
          ...
          , rdate  = Math.max( most_recent( context[ key ].items )
                             , _template.mtime
                             , this.settings.mtime )
          ;
    ...
    [ 'rss', 'atom' ].forEach(function ( feed_type ) {
      var feed_file = this.router( feed_type + '_feed', null, 'index.xml' )
        ...
        , trigger_date = Math.max( most_recent( this.posts ), this.settings.mtime )
        ;
